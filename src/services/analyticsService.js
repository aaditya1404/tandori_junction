...
export const getPopularItems = async () => {
  ...
  try {
    ...
    if (!orderResult.success || !menuResult.success) {
      return [];
    }
    const sales = {};
    ...
    orderResult.orders.forEach((order) => {
      ...
      order.items?.forEach((item) => {
        ...
        sales[item.name] += Number(item.quantity || 0);
      }
      );
    }
    );

    return Object.entries(sales).map(([name, quantity]) => {
      const menuItem = menuItems.find((menu) => menu.name?.trim().toLowerCase() === name?.trim().toLowerCase());
      console.log("Menu items", menuItem)
      return {
        name,
        quantity,
        image: menuItem?.image || "/food-placeholder.jpg"
      };
